...
void CEasySound::Unload(CSound *sound)
{
	...
	for (cs = m_listSound.begin(); cs != m_listSound.end(); ++cs) {
		if ((*cs)->GetSoundID() == sound->GetSoundID()) {
			m_listSound.erase(cs);
			//delete (*cs);  //KM 2/9/2016 Debug Assertion that iterator not dereferencable, maybe from here? assertion doesn't happen when this commented out but sound is still messed up.
			return;
		}
	}
}
